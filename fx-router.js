...
const options=['Volume','Pitch','Attack','Release','Filter Cutoff','Drive','Delay Mix','Delay Feedback','Stutter Rate','Stutter Depth','Delay Gate Rate','Delay Gate Depth','Wobble Rate','Wobble Depth','Tremolo Rate','Tremolo Depth','Pitch Jitter Rate','Pitch Jitter Depth','Tape Stop','Bitcrush','Sample Crush','Glitch Repeat','Darken','Radio Filter'];
const defaults=['Volume','Pitch','Attack','Release','Filter Cutoff','Drive','Delay Mix','Delay Feedback'];
const state={guard:false,assign:JSON.parse(localStorage.getItem('choppaFxAssignV3')||'null')||defaults,timers:new Map(),vals:{stutterRate:6,stutterDepth:.85,delayGateRate:4,delayGateDepth:.8,wobbleRate:2,wobbleDepth:.45,tremoloRate:4,tremoloDepth:.5,jitterRate:6,jitterDepth:1.5}};
...
switch(a){
case 'Volume':set('volume',v);break;case 'Pitch':set('pitch',-12+v*24);break;case 'Attack':set('attack',v*.2);break;case 'Release':set('release',.01+v*1.19);break;case 'Filter Cutoff':set('filter',200+v*19800);break;case 'Drive':set('drive',v);break;case 'Delay Mix':set('delay',v*.8);break;case 'Delay Feedback':set('feedback',v*.75);break;
...
case 'Wobble Rate':state.vals.wobbleRate=.5+v*7.5;restartModulation();break;case 'Wobble Depth':state.vals.wobbleDepth=v;restartModulation();break;
case 'Tremolo Rate':state.vals.tremoloRate=.5+v*11.5;restartModulation();break;case 'Tremolo Depth':state.vals.tremoloDepth=v;restartModulation();break;
case 'Pitch Jitter Rate':state.vals.jitterRate=1+v*15;restartModulation();break;case 'Pitch Jitter Depth':state.vals.jitterDepth=.2+v*5.8;restartModulation();break;
case 'Tape Stop':set('pitch',-12*v);break;case 'Bitcrush':set('drive',v);break;case 'Sample Crush':set('filter',20000-v*18500);break;case 'Glitch Repeat':set('feedback',v*.72);break;case 'Darken':set('filter',20000-v*19000);break;case 'Radio Filter':set('filter',1200+v*5000);break;}
state.guard=false}
function build(){const cards=[...document.querySelectorAll('.knob')];cards.forEach((card,i)=>{let s=card.querySelector('.fxAssign');if(!s){s=document.createElement('select');s.className='fxAssign';card.appendChild(s)}s.innerHTML='';options.forEach(name=>{const o=document.createElement('option');o.value=name;o.textContent=name;s.appendChild(o)});s.value=state.assign[i]||defaults[i];s.onchange=()=>{state.assign[i]=s.value;localStorage.setItem('choppaFxAssignV3',JSON.stringify(state.assign));clearSlot(i);apply(i,norm(el(ids[i])));restartModulation()}});
