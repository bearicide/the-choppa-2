const VERSION='stable-slice-loop-v1';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    ...
    const fixed="function trigger(i,vel=1,gate=false,retrigger=false){ensureAudio();if(!buffer||!slices[i]){beep(i,vel);hit(i);return null}if(choke&&!gate&&!retrigger)stopActive(true);const[s,e]=slices[i],entry=makeSource(s,Math.max(.08,e-s),vel,gate,false);if(!entry)return null;entry.pad=i;active.push(entry);if(!retrigger)hit(i);led(i,looping.has(i)?'loop':'play');entry.src.onended=()=>{active=active.filter(x=>x.src!==entry.src);if(killed.has(entry.src)){led(i,'off');return}if(looping.has(i))trigger(i,vel,false,true);else led(i,'off')};return entry}";
    html=html.replace(broken,fixed);
    html=html.replace(/<script src="\.\/fx-router\.js[^\"]*"><\/script>/g,'');
